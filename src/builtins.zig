...
pub fn sq(all: *ReservedBumpAllocator, result_dest: ?[]f64, args: *[1]Value) Value {
    ...
    switch (a) {
        ...
        .array => |array| {
            ...
            return ReturnArray(all, checkpoint, result);
        },
    }
    @panic("not implemented");
}

fn ReturnArray(all: *ReservedBumpAllocator, checkpoint: usize, result: types.Array) Value {
    ...
    if (compact_bytes) |bytes| {
        ...
        const final_bytes = all.allocator().alignedAlloc(u8, types.Array.allocationAlignment(), bytes.len) catch @panic("out of memory");
        std.mem.copyForwards(u8, final_bytes, bytes);
        return .{ .array = types.Array.fromCompactAllocation(final_bytes, depth, size) };
    }

    const final_data = if (all.isNextAllocation(@ptrCast(result.data.ptr)))
        blk: {
    ...
    const final_meta = if (all.isNextAllocation(meta_bytes.ptr))
        blk: {
        ...
        types.Metadata.initWithShape(all, .Exclusive, result.shape());
